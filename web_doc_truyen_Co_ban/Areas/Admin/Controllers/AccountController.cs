...
namespace web_doc_truyen_Co_ban.Areas.Admin.Controllers
{
    ...
    public record AdminLoginDto(string Email, string Password, bool RememberMe);
    public record AdminRegisterDto(string Username, string Email, string Password, string ConfirmPassword);
    [Area("Admin")]
    ...
    public class AccountController : Controller
    {
        ...
        //    var redirectUrl = customUser?.Email == "admin@gmail.com"
        ...
        public async Task<IActionResult> Login(AdminLoginDto dto)
        {
            ...
            if (customUser != null)
            {
                ...
                // ✅ Chỉ cho Admin/HỗTrợViên/NguoiDangTruyen vào
                var allowedRoles = new[] { "Quản Trị Viên", "Hỗ Trợ Viên", "Người Đăng Truyện" };
                if (customUser.Role == null || !allowedRoles.Contains(customUser.Role.Name))
            ...
            if (customUser != null)
            {
                customUser.LastLoginAt = DateTime.UtcNow;
                await _dbcontext.SaveChangesAsync();
                await CreateSessionAsync(customUser.Id, dto.RememberMe);
                SetHttpSession(customUser);
            }
        ...
        {
            // Admin AccountController - SetHttpSession
            HttpContext.Session.SetString("Admin_UserId", user.Id.ToString());
